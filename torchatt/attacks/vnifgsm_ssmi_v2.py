...
class VNIFGSM(Attack):
    ...
    def forward(self, images, labels, seg_image, cam_image):
        r"""
        Overridden.
        """
        images = images.clone().detach().to(self.device)
        seg_image = seg_image.clone().detach().to(self.device)
        ...
        for _ in range(self.steps):
            ...
            # adv_images = adv_images.detach() + self.alpha*grad.sign()*seg_image*cam_image
            adv_images = adv_images.detach() + self.alpha * grad.sign() * cam_image
