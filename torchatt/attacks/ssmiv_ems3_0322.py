...
from ..ssmi_v2_ems3_attack import Attack
...
class VNIFGSM(Attack):
    ...
    def __init__(self, model1, model2, model3, eps=8/255, alpha=2/255, steps=10, decay=1.0, N=5, beta=3/2):
        super().__init__("VNIFGSM", model1, model2, model3)
        self.eps = eps
        self.steps = steps
        self.decay = decay
        self.alpha = alpha
    ...
    def forward(self, images, labels, cam_image):
        ...
        for _ in range(self.steps):
            ...
            outputs1, outputs2, outputs3 = self.get_logits(nes_images)
            ...
            momentum = (grad1 + grad2 + grad3 ) / 3
            adv_grad = (adv_grad1 + adv_grad2 + adv_grad3 ) / 3
            ...
            for _ in range(self.N):
                ...
                outputs1, outputs2, outputs3 = self.get_logits(neighbor_images)
            ...
            GV_grad = (GV_grad1 + GV_grad2 + GV_grad3 ) / 3
            # obtaining the gradient variance
            v = GV_grad / self.N - adv_grad

            sg = (grad1 + grad2 + grad3).sign()
