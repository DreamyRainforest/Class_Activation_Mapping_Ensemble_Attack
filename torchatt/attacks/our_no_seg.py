...
class modify_DIFGSM(Attack):
    ...
    Examples::
        >>> attack = torchatt.DIFGSM(model, eps=8/255, alpha=2/255, steps=10, decay=0.0, resize_rate=0.9, diversity_prob=0.5, random_start=False)
        >>> adv_images = attack(images, labels)

    """

    def __init__(self, model, net, gama=0.5, gamaa=0.5, eps=8/255, alpha=2/255, steps=10, decay=0.0,
    ...
    def forward(self, images, labels, seg_image, temp_image_name, attack_layer_idx=-1, internal=[]):
        ...
        moment = torch.zeros_like(images).detach().to(self.device)
        if self.targeted:
        ...
        for _ in range(self.steps):
            ...
            # adv_images = adv_images.detach() + self.alpha*grad.sign()*seg

            adv_images = adv_images.detach() + self.gama * self.alpha * grad.sign() - self.gamaa * self.alpha * grad_DR.sign_()
