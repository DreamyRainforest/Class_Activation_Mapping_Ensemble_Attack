...
class VNIFGSM(Attack):
    ...
    def forward(self, images, labels, cam_image, k, attack_layer_idx=-1, internal=[]):
        r"""
        Overridden.
        """
        ...
        for _ in range(self.steps):
            ...
            # Calculate loss
            if self.targeted:
                cost = -loss(outputs, target_labels) - torch.norm(cam_image-images)
            else:
            ...
            adv_images = adv_images.detach() + self.alpha * 0.75 * grad.sign() * k * cam_image
